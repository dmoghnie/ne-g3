...
use clap::{arg, builder::PossibleValue, command, value_parser, Parser, ValueEnum};
use log::Level;
...
struct Cli {
    ...
    #[clap(short, long, default_value_t = String::from("127.0.0.1:3002"))]
    api_endpoint: String,
}
...
fn main() {
    ...
    match cli.api_endpoint.parse() {
        Ok(socket) => {
            let mut rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                if let Err(e) = api::start_api_server(socket).await{
                    log::warn!("Failed to start api server : {}", e)
                }
            });
        }
        Err(_) => {
            panic!("Invalid address and port")
        }
    };
