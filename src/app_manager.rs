...
impl MessageHandler for AppManagerinator {
    fn process(&mut self, msg: usi::Message) -> bool {
        log::info!("->AppManager : state ({:?}) : msg({:?})", self.state, msg);
        ...
        log::info!("<-AppManager : state ({:?})", self.state);
        return true;
    }
}

impl AppManagerinator {
    ...
    fn send_cmd(&self, msg: usi::OutMessage) -> Result<(), AppManagerError> {
        match self.cmd_tx.send(usi::Message::UsiOut(msg)) {
            Ok(_) => Ok(()),
            Err(e) => {
                log::warn! ("Send cmd: {}", e);
                Err(AppManagerError::SendError(e))
            },
        }
    }
}
